twitterApp = angular.module('twitterApp', ['ngMaterial', 'btford.socket-io'])
  .factory('socket', function(socketFactory) {
    ...
    function spawnSearch(q, tab) {
      ...
      //if old tweets exist add to initial result
      socket.on('past' + q, function(tweet){
        console.log(q, tweet.statuses);
        $scope['tweets_' + q] = tweet.statuses;
        updateScope(tab)
      });

      //event listener for new tweets containing query
    ...
    //checks to see if query exists in one of the threads running
    function dupes(tab) {
      var tabs = $scope.tabs;
      for (var j = 0; j < tabs.length; j++) {
        if (tab.q == tabs[j].q) {
